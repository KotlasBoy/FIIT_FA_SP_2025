...
protected:
    ...
    friend class __detail::bst_immpl<tkey, tvalue, compare, tag>;
...
public:
    ...
    {
    protected:
        node* _data;        /** If iterator == end or before_begin _data points to nullptr, _backup to last node    */
        node* _backup;
    ...
    public:
        ...
        reference operator*() const;            /** Throws exception if end */
        pointer operator->() noexcept;          /** UB if iterator points to end    */
...
public:
    ...
    explicit binary_search_tree(iterator begin, iterator end, const compare& cmp = compare(),       //дерево из другого контейнера
                                pp_allocator<value_type> alloc = pp_allocator<value_type>(), 
                                logger* log = nullptr);

    template<std::ranges::input_range Range>
    explicit binary_search_tree(Range&& range, const compare& cmp = compare(),                      //работа с любыми диапазонами
                                pp_allocator<value_type> alloc = pp_allocator<value_type>(),
                                logger* log = nullptr);


    binary_search_tree(std::initializer_list<std::pair<tkey, tvalue>> data, const compare& cmp = compare(), //initializer_list
                       pp_allocator<value_type> alloc = pp_allocator<value_type>(), 
                       logger* log = nullptr);

public:
    
    binary_search_tree(const binary_search_tree &other);        // rule of 5
...
public:
    ...
    tvalue& at(const tkey& key);                //безопасный доступ к значению
    ...
    template<std::ranges::input_range R>                // инсёрт рейндж
    void insert_range( R&& rg );

    template<class ...Args>
    std::pair<infix_iterator, bool> emplace(Args&&...args);     //создать элементы в дереве

    infix_iterator insert_or_assign(const value_type&);         //вставка / перезапись
    infix_iterator insert_or_assign(value_type&&);

    template<std::input_iterator InputIt >
    void insert_or_assign(InputIt first, InputIt last);         //для итераторов
...
namespace __detail          //отвечает за шаблонный метод
{
    template<typename tkey, typename tvalue, typename compare, typename tag>
    class bst_impl
    {
...
template<typename tkey, typename tvalue, typename compare, typename tag>
void __detail::bst_impl<tkey, tvalue, compare, tag>::swap(binary_search_tree<tkey, tvalue, compare, tag> &lhs,      //свап деревьев
                                                                   ...
                                                                   pp_allocator<typename binary_search_tree<tkey, tvalue, compare, tag>::value_type> alloc, logger *log)
{
    ...
    {
        emplace(it->first, it->second);     //<tkey, tvalue>
    }
}
...
template<typename compare, typename U, typename iterator>                   // class template argument deduction; компилятор сам поймёт типы по итераторам
...
binary_search_tree<tkey, tvalue, compare, tag>::node::node(node* parent, Args&& ...args) :
                data(std::forward<Args>(args)), parent(parent), left_subtree(nullptr), right_subtree(nullptr) {}    //forward повышает эффективность передачи
...
binary_search_tree<tkey, tvalue, compare, tag>::prefix_iterator::operator++() & noexcept
{
    ...
    else if (_data->right_subtree != nullptr)
    {
        ...
        while (parent != nullptr && (current == parent->right_subtree || parent->right_subtree == nullptr))
        {
            current = parent;
...
binary_search_tree<tkey, tvalue, compare, tag>::prefix_iterator::operator--() & noexcept
{
    ...
    else if (_data == parent->right_subtree)
    {
...
{
    prefix_const_iterator tmp = *this;
    ++_base;
    return *this;
}
...
binary_search_tree<tkey, tvalue, compare, tag>::prefix_const_reverse_iterator::operator--() & noexcept
{
    ++_base; // назад в обр, вперед в прямом
    return *this;
...
binary_search_tree<tkey, tvalue, compare, tag>::postfix_iterator::operator++() & noexcept
{
    ...
    else if (_data == parent->right_subtree || parent->right_subtree == nullptr)
    {
        _data = parent;
    }
    else
...
binary_search_tree<tkey, tvalue, compare, tag>::postfix_iterator::operator--() & noexcept
{
    if (_data == nullptr)
    {
        ...
        {
            _data = _backup;
            while (_data->right_subtree != nullptr || _data->left_subtree != nullptr)
            {
                if (_data->right_subtree != nullptr)
                {
                    _data = _data->right_subtree;
    ...
    else
    {
        ...
        while (parent != nullptr &&
               (current == parent->left_subtree || parent->right_subtree == nullptr))
        {
            current = parent;
            parent = parent->parent;
        }
...
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(const compare &comp, pp_allocator<value_type> alloc,
                                                                   logger *log) : compare(comp), _allocator(alloc),
                                                                                  _logger(log), _root(nullptr) {}
...
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(Range &&range, const compare &cmp,
                                                                   pp_allocator<value_type> alloc, logger *log) : compare(cmp), _allocator(alloc), _logger(log), _root(nullptr)
{
    ...
    : compare(cmp), _allocator(alloc), _logger(logger), _root(nullptr)
{
    for (auto &&pair : data)
    {
        emplace(std::forward<decltype(pair)>(pair));
    }
}
...
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(const binary_search_tree &other)
    : compare(other.compare), _logger(other._logger), _allocator(other._allocator), _root(nullptr), _size(0)
{
    if (other._root)
    {
...
binary_search_tree<tkey, tvalue, compare, tag>::insert(const value_type& value)
{
    ...
    {
        parent->right_subtree = new_node; // вставка в ппд
    }
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
typename binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator
binary_search_tree<tkey, tvalue, compare, tag>::lower_bound(const tkey& key)    // first elem to be >= key
{
    node *current = _root;
    node *result = nullptr;
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
typename binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator
binary_search_tree<tkey, tvalue, compare, tag>::upper_bound(const tkey& key)        // first elem to be > key
{
    node *current = _root;
    node *result = nullptr;
    ...
    while (current != nullptr)
    {
        if (compare::operator()(key, current->data.first)) // key < current
        {
            result = current;                    
            current = current->left_subtree;     
        }
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
typename binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator
binary_search_tree<tkey, tvalue, compare, tag>::erase(infix_iterator pos)           //TODO:FIXME:
{
    if (pos == end())
    {
        return end();
    ...
    node* to_delete = pos._data;
    node* next_node = nullptr;

    if (to_delete->right_subtree != nullptr)
    {
        next_node = to_delete->right_subtree;
        while (next_node->left_subtree != nullptr)
        {
            next_node = next_node->left_subtree;
        }
    }
    else
    {
        ...
        while (parent != nullptr && current == parent->right_subtree)
        {
            current = parent;
            parent = parent->parent;
        }
    ...
    __detail::bst_impl<tkey, tvalue, compare, tag>::erase(*this, to_delete);
    --_size;

    return next_node != nullptr ? infix_iterator(next_node) : end();
...
binary_search_tree<tkey, tvalue, compare, tag>::begin_prefix() noexcept
{
    if (_root == nullptr)
    {
        return prefix_iterator(nullputr);
    }
...
binary_search_tree<tkey, tvalue, compare, tag>::begin_prefix() const noexcept
{
    if (_root == nullptr)
    {
        return prefix_const_iterator(nullputr);
    }
...
binary_search_tree<tkey, tvalue, compare, tag>::begin_postfix() noexcept
{
    ...
    node * current = _root;
    while (current->left_subtree != nullptr || current->right_sibtree != nullptr)
    {
...
binary_search_tree<tkey, tvalue, compare, tag>::end_postfix() noexcept
{
    if (_root == nullptr)
        return postfix_iterator(nullptr);
    return postfix_oiterator(nullptr, _root);
}
...
{
    if (subtree_root != nullptr && subtree_root->right_subtree != nullptr && subtree_root->right_subtree->left_subtree
        != nullptr)
    {
...
{
    if (subtree_root != nullptr && subtree_root->left_subtree != nullptr && subtree_root->left_subtree->right_subtree !=
        nullptr)
    {
...
{
    if (subtree_root != nullptr && subtree_root->right_subtree != nullptr && subtree_root->right_subtree->right_subtree
        != nullptr)
    {
        small_left_rotation(subtree_root);
        small_left_rotation(subtree_root);
...
{
    if (subtree_root != nullptr && subtree_root->left_subtree != nullptr && subtree_root->left_subtree->left_subtree !=
        nullptr)
    {
        small_right_rotation(subtree_root);
        small_right_rotation(subtree_root);

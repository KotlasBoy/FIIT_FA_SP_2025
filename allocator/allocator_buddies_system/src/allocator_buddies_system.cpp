...
{
    trace_with_guard("allocator_buddies_system::destructor");

    std::byte *base = static_cast<std::byte *>(_trusted_memory);

    unsigned char space_size_power = *reinterpret_cast<unsigned char *>(base + allocator_metadata_size - sizeof(unsigned char));

    if (space_size_power == 0)
    {

        this->error_with_guard("allocator_buddies_system: invalid space_size_power == 0 on destruction");
        return;
    }

    size_t total_size = (1ULL << space_size_power) + allocator_metadata_size;

    std::mutex *mutex_ptr = reinterpret_cast<std::mutex *>(base + allocator_metadata_size);
    mutex_ptr->~mutex();
    ...
    _trusted_memory = nullptr; // на всякий случай
    ...
    allocator_with_fit_mode::fit_mode allocate_fit_mode)
{
    if (space_size < min_k)
    {
        throw std::logic_error("allocator_buddies_system: requested size too small");
    }
    ...
    *reinterpret_cast<class logger **>(ptr) = logger;
    ...
    trace_with_guard("allocator_buddies_system::constructor");
    ...
    *reinterpret_cast<unsigned char *>(ptr) = static_cast<unsigned char>(space_size_power);
    ...
    debug_with_guard("Initial block state: avail " +
                     std::to_string(1ULL << first_block->size));
}
// O(n)
...
void allocator_buddies_system::do_deallocate_sm(void *at)
{
    ...
    while (meta->size < max_k)
    {
        ...
        // бадди выходит за пределы пула-останавливаем объединение
...
allocator_buddies_system::allocator_buddies_system(const allocator_buddies_system &other)
{
    ...
    new (reinterpret_cast<uint8_t *>(_trusted_memory) +
         sizeof(logger *) + // пересоздаем нафиг мьютекс
         sizeof(allocator_dbg_helper *) +
         sizeof(fit_mode) +
...
{
    auto base = static_cast<std::byte *>(_trusted_memory);
    ...
    auto raw = base + offset_to_mutex;
    auto mtx = reinterpret_cast<std::mutex *>(raw);
    return *std::launder(mtx);
}
